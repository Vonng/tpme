...
在{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}中，我们<a id="id807"></a>找到了自己的客户。我们想弄清楚客户是谁、他们的问题是什么。通过这个过程，我们打磨出了一份产品论题（product thesis）：即我们对“什么才是这群人眼中的优秀产品”的高层主张。
...
{{< xref fig="7-1" page="/ch7" anchor="product_discovery_workflow" >}}图 7-1{{< /xref >}} 展示了一个围绕<a id="pdis07"></a>产品发现组织流程的示例，它分成三份文档：

- **产品简报（Product brief）**：一份<a id="id817"></a>愿景文档，用于让所有人对目标达成一致。有时也叫“问题简报（problem brief）”或“一页纸（one-pager）”。最少应包含：产品论题/反命题与目标受众（{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}）、要追踪的目标或产品指标（{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}），以及稍后会定义的*北极星场景*。在<a id="id818"></a>双钻模型（Double Diamond）术语里，它的目标是为发现阶段清障。
...
{{< fig num="7-1" id="product_discovery_workflow" src="/fig/tpme_0701.png" caption="一个产品发现流程示例" alt=".一个产品发现流程示例" />}}
...
如{{< xref page="/ch1" anchor="ch01_foundations_prod" >}}第 1 章{{< /xref >}}所述，团队应该通过寻找情节漏洞来给选中的故事“调试”。有些故事是不是跳过了<a id="id854"></a>用户旅程中的关键步骤？是不是对用户做了英雄式假设？是不是忽略了关键细节？团队中有些工程师也许不擅长从零写故事，但他们在这一步可能非常强，这同样很有价值。
...
如果我们的大脑里也有类似这些“数据库”，那么从发现阶段走向设计阶段的过程，就是一次从“时间/故事表示”到“图/<a id="id860"></a>系统表示”的<a id="id859"></a>*大重排*（great reindexing）。{{< xref fig="7-2" page="/ch7" anchor="great_reindexing_story_view" >}}图 7-2{{< /xref >}} 与 {{< xref fig="7-3" page="/ch7" anchor="great_reindexing_system_view" >}}图 7-3{{< /xref >}} 展示了同一产品的两种表示：

{{< fig num="7-2" id="great_reindexing_story_view" src="/fig/tpme_0702.png" caption="故事视图" alt="故事视图" />}}

{{< fig num="7-3" id="great_reindexing_system_view" src="/fig/tpme_0703.png" caption="系统视图" alt="系统视图" />}}
...
{{< xref tbl="7-1" page="/ch7" anchor="helpy_use_case_compendium" >}}表 7-1{{< /xref >}} 展示了用例汇编。

{{< tbl num="7-1" id="helpy_use_case_compendium" caption="Helpy 的用例汇编" >}}
| 需求 | 里程碑 | 人物画像 | 北极星场景 |
| --- | --- | --- | --- |
...
优先级是我们工作中最难且影响最大的部分之一，最后常常会有点混乱。每个人都有自己的“重要性”判断，也很容易分心。看看{{< xref tbl="7-1" page="/ch7" anchor="helpy_use_case_compendium" >}}表 7-1{{< /xref >}}：这张表的规模还不到我把全部内容写全时的一半。再叠加多里程碑规划、组织约束和代码库约束，确实会令人不知所措。
...
{{< xref fig="7-4" page="/ch7" anchor="bicycle_utility_function" >}}图 7-4{{< /xref >}} 展示了自行车的效用函数：

{{< fig num="7-4" id="bicycle_utility_function" src="/fig/tpme_0704.png" caption="随功能增加而变化的自行车效用函数" alt="自行车效用函数" />}}
...
我<a id="id877"></a>把它叫做*替代产品之上价值*（Value over Replacement Product, VORP）[^id878]。计算 VORP 时，需要把市场上已有产品，或你正在构建产品的旧版本价值，从你的效用函数里扣掉。自行车案例的曲线大致如{{< xref fig="7-5" page="/ch7" anchor="bicycle_vorp" >}}图 7-5{{< /xref >}}。
...
{{< fig num="7-5" id="bicycle_vorp" src="/fig/tpme_0705.png" caption="随功能增加而变化的自行车“相对替代产品价值”函数" alt="自行车 VORP" />}}
...
正如我在{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}提到的，我们可以找到被现有方案服务不足的目标受众，他们会为了“真正为自己而做”的东西容忍功能缺失。

另一条常见的更优做法是<a id="id880"></a><a id="id881"></a><a id="id882"></a>迭代产品，正如{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}所讲。迭代时，我们会暂时忽略一两条残酷真相，但沿途仍在学习。
...
我们还可以进一步收窄早期目标受众。最常见做法是限制为可参与内部试用（dogfooding）的员工（{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}），或愿意容忍早期版本以换取产品影响力/白手套支持的设计伙伴。另一个选择是限定单平台用户，比如 Android 而非 iOS，或者限定不太需要引导的高级用户。
...
我们的大多数员工家里都用 Kabletown，所以首个里程碑先限制在 Kabletown 员工，他们更宽容，也会给出更有质量的反馈（{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}）。这样我们可以先发布 Helpy 并从中学习，再去做完整打磨。面向员工的首发因此是 MVP，而非 MLP。
...
{{< xref tbl="7-2" page="/ch7" anchor="helpy_prioritized_use_case_compendium" >}}表 7-2{{< /xref >}} 展示了按优先级排序后的用例汇编，以及我为何做出某些看起来反直觉优先级的脚注说明：
...
| …​ |  |  |  |
{#helpy_prioritized_use_case_compendium num="7-2" caption="Helpy 的优先级用例汇编"}
...
- 用户流程应足够细，以便我们围绕关键交互问题展开讨论（{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}），并从中提炼清晰系统规格（{{< xref page="/ch9" anchor="ch09_product_architecture" >}}第 9 章{{< /xref >}}）。
...
在{{< xref page="/part-iv" anchor="part4" >}}第四部分{{< /xref >}}中，我们会把注意力转向这些组件与系统的设计。
...
2025 年代理设计中的一个现实是：对于复杂代理，采用“多个专长代理混合”通常比“一个全能大代理”错误更少。在本例里，我们可以做一个技术支持 assistant 和一个销售 assistant，再由 Helpy 作为礼宾代理（concierge agent）负责对外沟通、规划，并把任务分派给各 assistant，如{{< xref fig="7-6" page="/ch7" anchor="mixture_of_agents" >}}图 7-6{{< /xref >}}所示。

{{< fig num="7-6" id="mixture_of_agents" src="/fig/tpme_0706.png" caption="两个专长助手与一个代理的混合模式" alt="由两个专长助手与一个代理组成的混合体。" />}}
