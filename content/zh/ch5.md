...
一个直接结果是：用户更快拿到更多价值。即便产品总体演进轨迹不变，{{< xref fig="5-1" page="/ch5" anchor="value_time" >}}图 5-1{{< /xref >}} 也说明，在任意时点，用户获得的价值都更高。

{{< fig num="5-1" id="value_time" src="/fig/tpme_0501.png" caption="随时间推移交付给用户的产品价值" alt="用户随时间获得的产品价值" />}}
...
- 我们基于生产中观察到的现象（或预期会发生的现象），设计{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}讨论的测试。
- 我们加载仿真（{{< xref page="/ch9" anchor="ch09_product_architecture" >}}第 9 章{{< /xref >}}），用真实请求或合理拟真请求去压测数字孪生。
- 我们记录摩擦日志（同样见{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}），捕捉“使用产品到底是什么体验”。
...
- 一个数据库抽象层 `EntSchema`，<a id="id653"></a>让模型开发更轻松且强大。我会在{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}详细介绍它。
...
- 始终写自动化测试（{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}），尤其要重点覆盖最关键场景。关键场景有扎实覆盖，会带来信心：改动不会打断用户最重要的交互。
...
我记得曾向一位 Oculus 工程师展示 EntSchema（{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}会讲），它能让新增字段代码生成与数据库回填变得非常安全且轻松。我当时主张：可以把 PR 里那些过度预判的字段删掉，因为以后随时都能再加。
...
重写的后续效应非常明显。接下来几年里，团队能在新栈上快速设计并落地大量新功能。在{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}和{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}中，我会展示团队如何利用迭代方式，在设计与<a id="id682"></a>优先级上更聚焦，从而做出更高质量产品。
...
在{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}里，我们通过内部试用和摩擦日志，在产品生命周期早期给自己反馈。
...
在{{< xref page="/ch9" anchor="ch09_product_architecture" >}}第 9 章{{< /xref >}}中，我们会讨论非功能性需求对应的指标。我们不仅要给单个操作做埋点，还要给用户流程做埋点，让指标更贴近用户结果。
...
在向管理层推销项目时，你可以在产品论题（{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}）或产品简报（{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}）里，把项目与 KPI 绑定起来。
...
{{< xref tbl="5-1" page="/ch5" anchor="tactical_and_strategic_metrics" >}}表 5-1{{< /xref >}} 对不同指标取舍做了简洁总结：

{{< tbl num="5-1" id="tactical_and_strategic_metrics" caption="战术指标与战略指标倾向对比" >}}
| 战术指标 | 战略指标 |
| --- | --- |
| 是领先指标 | 是滞后指标 |
| 靠聚焦即可推动 | 靠创造力推动 |
