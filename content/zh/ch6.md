...
产品思维工程师<a id="id748"></a><a id="id749"></a>最关键的视角切换是：为某个具体的人而构建。了解客户后，我们才能选择彼此互补的功能组合（我们会在{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}详细设计），并把精力优先投入真正需要的需求上，例如延迟、可扩展性与安全性（{{< xref page="/ch9" anchor="ch09_product_architecture" >}}第 9 章{{< /xref >}}）。
...
在这一章，你会认识你的目标受众。眼下我先聚焦于你与潜在客户的初次互动：访谈他们、了解他们，并判断谁属于你的目标受众。（对比而言，在{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}里，我们是在产品较后期版本上迭代，咨询的是现有用户。）

你会用这些信息构建人物画像（以及非人物画像），以指导整个团队的工作。这些方法能帮助你形成清晰愿景，并在重大决策上做出正确选择。

### 真实用户，而非稻草人 {#id99}

正如我在{{< xref page="/ch1" anchor="ch01_foundations_prod" >}}第 1 章{{< /xref >}}提到的，真实的人类有一些普遍特征<a id="id750"></a><a id="id751"></a>，例如健忘、只会把有限脑力分配给你的产品、而且很忙。这些经验法则通常有用，但它们和所有经验法则一样，不总是对的，也远远不够具体。如果你没有完整的人物画像来支撑决策，就很容易不小心为自己而构建，或为你*上一个*产品服务过的人物画像而构建。
...
- **你的克隆体（Your Clone）**：和你一样了解系统，因此不需要任何发现机制。由于熟悉内部实现，他对反直觉行为的容忍度异常高。这个“克隆体”正体现了{{< xref page="/ch2" anchor="ch02_naming" >}}第 2 章{{< /xref >}}提到的“知识诅咒”陷阱。
- **你爸妈（Mom or Dad）**：是真实的人，但很可惜，你最多也就这一个或两个样本。

我把这些叫作“稻草人用户”，和我们在{{< xref page="/ch1" anchor="ch01_foundations_prod" >}}第 1 章{{< /xref >}}批评动机薄弱场景时讨论的是一类问题。
...
- 在我于{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}讲过的支持场景中，直接问用户是否愿意接受访谈。若你此前帮到过他们，他们通常很可能答应。
...
把 CDI 想象成一个漏斗。开头你在用户世界里，聊他们的故事、问题和愿望，你在锚定他们的人物画像。随着访谈推进，你逐步把他们带到你的产品世界里，对产品提出越来越聚焦的问题，如{{< xref fig="6-1" page="/ch6" anchor="customer_interview_funnel" >}}图 6-1{{< /xref >}}所示。

{{< fig num="6-1" id="customer_interview_funnel" src="/fig/tpme_0601.png" caption="客户发现访谈漏斗" alt="A time-based graph of an interview progress." />}}
...
- 走一遍具体故事能帮你构建场景，指导后续设计工作，我会在{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}进一步讨论。
...
假设我们担心玩家不希望朋友看到自己的游戏行为，如{{< xref fig="6-2" page="/ch6" anchor="app_center_recommended" >}}图 6-2{{< /xref >}}所示。

{{< fig num="6-2" id="app_center_recommended" src="/fig/tpme_0602.png" caption="App Center 原型" alt="A prototype app center listing" />}}
...
以 App Center 为例：如果我们想判断星级评分是否有价值，用户可能以不同方式表达，而这些表达有不同信号强度，如{{< xref tbl="6-1" page="/ch6" anchor="star_ratings" >}}表 6-1{{< /xref >}}所示：

{{< tbl num="6-1" id="star_ratings" caption="星级评分反馈" >}}
| 提示问题 | 客户回应 | 信号强度 |
| --- | --- | --- |
...
在{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}中我们讲过，如何在用户引导下持续迭代产品。用户互动心态的一项关键支撑，就是你有一组可信任、可快速联系并获得反馈的人。
...
在{{< xref page="/ch1" anchor="ch01_foundations_prod" >}}第 1 章{{< /xref >}}中，我给出过场景角色的组成元素：
...
因此，广泛沟通并让所有人围绕目标受众达成一致非常关键。关于优先级，我会在{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}再展开<a id="id795"></a><a id="id796"></a>。

### 多画像产品 {#id114}

大多数<a id="TAmulti06"></a><a id="multides06"></a><a id="Pmulti06"></a>成功产品最终都会服务多种人物画像。

一个常见二分是重度用户与普通用户。我在{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}会讲如何同时满足这两类。Moby 是重度用户，和 Penny 对照明显，但二者都属于社交玩家这一大类。
