...
Bob 做了一个与现有应用结构契合度很高的原型（见{{< xref fig="1-1" page="/ch1" anchor="coffee_app_mockup" >}}图 1-1{{< /xref >}}）。他准备在应用底部新增一个 Favorites 菜单项，并增加一个展示用户收藏的新页面。页面右上角有一个 *+* 按钮，用来新增收藏。

{{< fig num="1-1" id="coffee_app_mockup" src="/fig/tpme_0101.png" caption="Bob 展示了他提议的移动应用改动。新增元素以阴影标出。" alt="咖啡点单应用原型" />}}
...
第三，她做了 A/B 测试：将首页新品推广替换为一对巨大的“立即下单”按钮，并露出第三个按钮顶部，提示页面可滚动（见{{< xref fig="1-2" page="/ch1" anchor="quick_order_mockup" >}}图 1-2{{< /xref >}}）。这个思路和 Bob 曾设想的类似，但他当时没能说服产品营销团队让出首页空间。
...
{{< fig num="1-2" id="quick_order_mockup" src="/fig/tpme_0102.png" caption="Alice 做了一个可滚动的快速下单页面原型，包含“立即下单”按钮" alt="快速下单页面原型" />}}
...
我们会在{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}讨论用户访谈。
...
我会在{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}讨论如何收集用户反馈，并把它转化成此类故事或用户原话。
...
像这样引导<a id="id325"></a><a id="id326"></a>产品设计的场景，被称为“北极星场景（north star scenarios）”，将在{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}讨论。

### 作为测试的场景 {#id279}

Alice 和 Bob 没聊到这一点，但她在编写自动化测试时也写了“场景测试”：它不只覆盖单个页面，还确保页面之间衔接正确，验证用户确实能走完整个常见流程。

场景测试会在{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}重点介绍。
...
首先，团队最好先就一个或多个*目标受众*达成一致，即你的应用会服务哪些人物画像，也可能面向哪些人物画像进行营销。我会在{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}带你定义目标受众。<a id="Pcreat01"></a>
...
人物画像能凸显为多群体构建产品时的差异与复杂性，是更精确思考用户的优秀工具。我会在{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}更深入讨论人物画像，讲清如何在团队沟通、优先级制定与设计思考中使用它。接下来我也会频繁引用这一概念。
...
我在日常中一直用“换鞋思考”和“选择性失忆”来共情用户。本书后面会频繁回到这个方法，例如在{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}写“摩擦日志”时。
...
当她后续进入更细设计时，会把这些高层步骤继续细化，并可能配上原型页面示意，形成故事板。我会在{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}讨论这种“用户流程（user flows）”。
...
- 在开发阶段，你会用场景来选择好名字（{{< xref page="/ch2" anchor="ch02_naming" >}}第 2 章{{< /xref >}}），并设计错误层级和可执行的优质错误消息（{{< xref page="/ch3" anchor="ch03_errors" >}}第 3 章{{< /xref >}}）；你还会把它们转化为场景测试（{{< xref page="/ch4" anchor="ch04_dogfooding" >}}第 4 章{{< /xref >}}）。
- 在发布后，你会从用户反馈中收集这些场景（{{< xref page="/ch5" anchor="ch05_user_feedback" >}}第 5 章{{< /xref >}}）；在起步阶段，你会通过“客户发现访谈”获取它们（{{< xref page="/ch6" anchor="ch06_target_aud" >}}第 6 章{{< /xref >}}）。
- 在设计阶段，你会基于“按优先级整理的场景全集”制定产品路线图，并形成更细的用户流程（{{< xref page="/ch7" anchor="ch07_product_discovery_sim" >}}第 7 章{{< /xref >}}）。
- 场景还会指导细粒度交互设计（{{< xref page="/ch8" anchor="ch08_interaction_design" >}}第 8 章{{< /xref >}}）和架构决策，例如用于生成负载模拟（{{< xref page="/ch9" anchor="ch09_product_architecture" >}}第 9 章{{< /xref >}}）。
