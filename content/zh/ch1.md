...
Tea++<a id="CSeff01"></a> 是一家拥有 300 家门店的连锁咖啡馆。它扩张过快，经营勉强维持，因此公司的目标是在极低预算下尽快提升订单量，避免关店。
...
Bob 查看了应用反馈组件里收到的用户意见。咖啡用户厌倦了每次下单都要重新设置定制项，也不喜欢在近期订单里翻找。有时想要的商品埋在包含多个商品的订单中，拆分起来很麻烦。

Bob 做了一个与现有应用结构兼容性很好的原型（见[图 1-1](/en/ch1#coffee_app_mockup)）。他准备在应用底部新增一个 Favorites 菜单项，并增加一个展示用户收藏的新页面。页面右上角有一个 *+* 按钮，用来新增收藏。
...
第一，他跳过了 Discover 阶段（我在序言提过的四个阶段中的第一步），直接进入数据库设计。
...
在 Discover 阶段，探索多种可能的故事线非常关键。

### 捕捉用户访谈的场景 {#id10}

如果<a id="id318"></a>你做过一些用户研究，场景可以是对用户反馈的虚构合成，能够简洁传达常见情境。
...
你<a id="id322"></a>可以构造故事，来展示使用现有产品的痛点。这个例子里，Alice 设计了一个带些荒诞感的小故事。
...
例如，下面是 Alice 的*改后*故事，不仅包含心形按钮，还把“门店”“收藏”“首页展示”三项一起串联：
...
一个<a id="id327"></a>场景是为激发产品批判性思考而设计的用户故事。它由两部分构成：一个角色，以及该角色在发现并使用产品或功能来解决问题时的行为模拟。
...
> Eliana 是一位乳糖不耐受的 Tea++ 新近用户，记得自己之前点过的饮料很好喝。她想复购，于是点进 Recent Orders，发现那杯叫 Tiger Chai Latte。她当时有一点空闲时间，也很了解自己“喜欢上某样东西就会反复点”的习惯，所以她进入 Favorites 菜单，点击 *+* 按钮，搜索 “tiger”，点击结果，重新配置定制项（豆奶、中杯），然后点击 OK。接着她再点击 “Order now”。
...
首先，团队最好先就一个或多个*目标受众*达成一致，即你的应用会服务、也可能会营销给哪些人物画像。我会在[第 6 章](/en/ch6#ch06_target_aud)带你定义目标受众。<a id="Pcreat01"></a>
...
> Eliana 是一位乳糖不耐受的 Tea++ 新近用户，记得自己喜欢 Tiger Chai Latte，并且知道自己还会再点几次。她点进 Favorites 菜单，点击 *+* 按钮，搜索 “tiger” 并选择这款饮料。然后她重放定制项（豆奶、中杯），再点 OK。当它出现在列表里后，她点击“Order now”。

这种故事往往会导向一种产品：上线后采用率不及预期，而用户为何不买账看起来又“莫名其妙”。

记住这些共性的一个方法是<a id="id340"></a>*shoe-shift*：也就是“换鞋思考”，把自己放到用户的位置，思考你在那个位置会知道什么、会怎么做。
...
> Eliana 是 Tea++ 的一位较新用户，记得自己之前点过一杯很好喝的饮料。在 Recent Orders 标签页里，她发现那杯叫 Tiger Chai Latte。她点击旁边的心形图标。系统把她带到 Favorites 菜单，并将其列在列表中。
...
一种<a id="id344"></a>情节漏洞是“跳步”。归根结底，你的产品就是用户旅程。用户不是静态地“使用”产品，而是在其中移动：发现、想起、学习、使用、承担结果。如果只盯界面本身（我们看得见的那部分），就会产生隧道视野。
...
场景<a id="id348"></a>应贯穿软件开发的每个方面：从最初发现（正如 Alice 在本章所做）直到实现规格。因此，本书很大部分内容都会依赖“故事”的力量。
...
- 在发布后，你会把场景作为用户反馈的一种组织方式（[第 5 章](/en/ch5#ch05_user_feedback)）；在起步阶段，你会通过“客户发现访谈”获取它们（[第 6 章](/en/ch6#ch06_target_aud)）。
